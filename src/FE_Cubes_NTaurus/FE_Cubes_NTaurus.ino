...
char cube_avoid_direction = 'R';  // Use R for right, red cube avoidance and L for left, green cube avoidance
int desiredSteering;
...
void calibrareceivedMessageU6050() {
  int numSamples = 1000;
  float sumZ = 0;
...
void wrireceivedMessageU6050(byte reg, byte value) {
  Wire.beginTransmission(MPU6050_ADDR);
  Wire.write(reg);
  Wire.write(value);
  Wire.endTransmission();
