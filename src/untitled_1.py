...
sensor.set_auto_exposure(False, exposure_us=8000)  # Adjusted exposure for better visibility
clock = time.clock()
...
orange_threshold = [(30, 80, 5, 45, 15, 75)]

# **Define Regions of Interest (ROI)**
image_height = sensor.height()
cubes_roi = (10, int(image_height * 0.6), sensor.width() - 20, int(image_height * 0.4))  # Bottom 40%
lines_roi = (5, int(image_height * 0.6), sensor.width() - 10, int(image_height * 0.4))  # Bottom 40%
...
def is_invalid_orange(orange_blob, red_blobs):
    ...
    for red_blob in red_blobs:
        if (
            ...
            if red_area >= orange_area * 1.5:  # 50% larger or more
                return "ignore_orange"
            elif orange_area >= red_area * 1.5:  # 50% larger or more
                return "ignore_red"
            else:
                return None

    return None  # No overlap, keep both
...
while True:
    ...
    red_cube = get_largest_blob([b for b in red_blobs if b.area() >= min_area])
    green_cube = get_largest_blob([b for b in green_blobs if b.area() >= min_area])
